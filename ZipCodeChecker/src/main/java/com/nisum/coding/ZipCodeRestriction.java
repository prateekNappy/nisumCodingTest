...
public class ZipCodeRestriction {
	...
	public static List<Range> findMinimumInputRestrictions(List<Range> inputRestrictions) {
			...
			if (Range.isValid(range)) {
				...
				if (outputRestrictions.size() == 0) {
					...
					if (RangeUtil.isSubset(range, outputRestrictions.get(outputRestrictions.size() - 1))) {
						...
						// If present Range is super set then, remove the latest in output and add the
						// present
						outputRestrictions.remove(outputRestrictions.size() - 1);
						outputRestrictions.add(range);
					} else if (RangeUtil.isInterceptingLeft(range,
							outputRestrictions.get(outputRestrictions.size() - 1))) {
						// If present range is extended to left, then remove latest from output and add
						// new keeping the start of present and end of latest
						...
						outputRestrictions.add(new Range(range.getStart(), end));
							...
							// If present range is extended to right, then remove latest from output and add
							// new keeping the end of present and start of latest
							outputRestrictions.get(outputRestrictions.size() - 1))) {
						start = outputRestrictions.get(outputRestrictions.size() - 1).getStart();
						outputRestrictions.remove(outputRestrictions.size() - 1);
						outputRestrictions.add(new Range(start, range.getEnd()));
					} else if (RangeUtil.isNonIntercepting(range,
							outputRestrictions.get(outputRestrictions.size() - 1))) {
